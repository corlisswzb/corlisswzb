...
var cur_query_delegate_group_params = undefined;
...
function get_basesetting() {
    post('../Ashx/sys_base.ashx', {
        ...
        bind_combogrid_custom($('#search_delegate_cu_id'));
        
        init_tab_delegate_group();
        init_order_tab();
        init_tab_fee_list();
        init_tab_order_cntr();
...
//刷新
function refresh_rpt_of_delegate_group() {
    cur_query_delegate_group_params = {
        rnd: Math.random(),
        action: 'get_rpt_of_delegate_group',
        ...
        delete_id: $('#search_delegate_cu_id').combobox('getValue'),
       
    };
    cur_query_sub_params = $.extend(true,{},cur_query_delegate_group_params);
 


    //只有月没有年，是不允许的 
    if (cur_query_delegate_group_params.beg_month != undefined && Number(cur_query_delegate_group_params.beg_month) > 0) {
        if (cur_query_delegate_group_params.beg_year == undefined ||
            Number(cur_query_delegate_group_params.beg_year) == 0) {
            $.messager.alert('错误','错误:无法理解的起始时间设置!','error');
            return;
        }
    }
    if (cur_query_delegate_group_params.end_month != undefined && Number(cur_query_delegate_group_params.end_month) > 0) {
        if (cur_query_delegate_group_params.end_year == undefined ||
            Number(cur_query_delegate_group_params.end_year) == 0) {
            $.messager.alert('错误', '错误:无法理解的截止时间设置!', 'error');
            return;
        }
    }
    
    post('../Ashx/lead_query.ashx', cur_query_delegate_group_params,
    function (data) {
        $("#tab_delegate_group").datagrid('loadData', data);
        $("#tab_delegate_group").datagrid('reloadFooter', [
        {
            cu_name: '', 
...
//初始表格
function init_tab_delegate_group() { 
    cur_query_delegate_group_params = {
        rnd: Math.random(),
        action: 'get_rpt_of_delegate_group',
    ...
    $("#tab_delegate_group").datagrid({
        ...
        checkOnSelect: true,
        columns: [[
            { field: 'cu_name', title: '委托单位', align: 'center',width:110, },
        ...
        onRowContextMenu: function (e, field, row) {
            e.preventDefault();
            $('#mm_of_datagrid').data('delete_id', row.cu_id);
...
function download_delegate_group() {
    var down_query_sub_params = $.extend(true,{}, cur_query_delegate_group_params);
    down_query_sub_params.action = 'download_rpt_of_delegate_group';
...
function win_view_of_order_list() {
    ...
    cur_query_sub_params.action = 'get_rpt_of_delegate_group_of_order_list';
...
function download_order_group() {
    ...
    down_query_sub_params.action = 'download_rpt_of_delegate_group_order_list';
...
function win_view_of_order_fee_list(rec_or_pay) {
    ...
    cur_query_sub_params.delete_id = $('#mm_of_datagrid').data('delete_id');
    cur_query_sub_params.action = 'get_rpt_of_delegate_group_of_fee_list';
...
function download_order_fee_group() {
    var down_query_sub_params = $.extend(true, {}, cur_query_sub_params);
    down_query_sub_params.action = 'download_rpt_of_delegate_group_order_fee';
    window.open('../Ashx/lead_query.ashx?' + parseParams(down_query_sub_params));
}
...
function win_view_of_order_cntr_list() {
    cur_query_sub_params.delete_id = $('#mm_of_datagrid').data('delete_id');
    cur_query_sub_params.action = 'get_rpt_of_delegate_group_of_cntr_list';
    cur_query_sub_params.delete_name = $('#mm_of_datagrid').data('cu_name');
    post('../Ashx/lead_query.ashx', cur_query_sub_params,
    function (data) {
...
function download_order_cntr_group() {
    var down_query_sub_params = $.extend(true, {}, cur_query_sub_params);
    down_query_sub_params.action = 'download_rpt_of_delegate_group_order_cntr';
    window.open('../Ashx/lead_query.ashx?' + parseParams(down_query_sub_params));
}