...
function get_basesetting() {
    ...
    //    basesetting = data;
        ...
        bind_combobox(select_years, $('#search_win_fee_dat_begin_year'), 'text', 'value', false);
        bind_combobox(select_years, $('#search_win_fee_dat_end_year'), 'text', 'value', false);
        bind_combobox(select_month, $('#search_win_fee_dat_begin_month'), 'text', 'value', false);
        bind_combobox(select_month, $('#search_win_fee_dat_end_month'), 'text', 'value', false);
        bind_combobox(basesetting.employe_list, $('#search_win_service_id'), 'u_real_name', 'u_id', true);

    //查询  
        bind_combobox(basesetting.employe_list, $('#search_win_operation_id'), 'u_real_name', 'u_id', false);
        bind_combobox(basesetting.employe_list, $('#search_win_sales_id'), 'u_real_name', 'u_id', false);
        bind_combobox(basesetting.employe_list, $('#search_win_record_id'), 'u_real_name', 'u_id', false);
        bind_combogrid_custom($('#search_win_fee_cu_id'));
...
function clear_query_tab_fee_list_params() {
    ...
    $("#tab_fee_list").datagrid('options').url = '../Ashx/checkaccount.ashx';
    $("#tab_fee_list").datagrid('load', cur_fee_list_query_params);
}
...
function open_order_fee_group_window() {
    $('#win_order_fee_group').window('open');

    refresh_order_fee_group();
}
//费用汇总表 
function init_tab_order_fee_group() {
    ...
    $("#tab_order_fee_group").datagrid({
        ...
        onDblClickCell: function (index, filed, value) {
            ...
            //所有
            if (filed == 'fee_amount_of_commit') {

            }
            //已销账
            if (filed == 'woa_amount_of_commit') {
                par.woa_flag = 1;
            }
            //已销账 不开票 
            if (filed == 'woa_amount_of_commit_of_noneinvoice') {
                par.woa_flag = 1;
                par.invoice_flag = 0;
            }
            //已销账 需票 小计
            if (filed == 'woa_amount_of_commit_of_invoice') {
                par.woa_flag = 1;
                par.invoice_flag = 1;
            }
            // 已销账 需票 未录票
            if (filed == 'woa_amount_of_commit_of_invoice_of_unrecord') {
                par.woa_flag = 1;
                par.invoice_flag = 1;
                par.record_invoice_flag = 0;
            }
            // 已销账 需票 已录票
            if (filed == 'woa_amount_of_commit_of_invoice_of_record') {
                par.woa_flag = 1;
                par.invoice_flag = 1;
                par.record_invoice_flag = 1;
            }
            // 未销账 小计 
            if (filed == 'unwoa_amount_of_commit') {
                par.woa_flag = 0;
            }
            // 未销账 不需票 小计 
            if (filed == 'unwoa_amount_of_commit_of_noneinvoice') {
                par.woa_flag = 0;
                par.invoice_flag = 0;
            }

            // 未销账 需票 小计 
            if (filed == 'unwoa_amount_of_commit_of_invoice') {
                par.woa_flag = 0;
                par.invoice_flag = 1;
            }

            // 未销账 需票 未录票 小计 
            if (filed == 'unwoa_amount_of_commit_of_invoice_of_unrecord') {
                par.woa_flag = 0;
                par.invoice_flag = 1;
                par.record_invoice_flag = 0;
            }
            ...
            // 未销账 需票 已录票 小计 
            if (filed == 'unwoa_amount_of_commit_of_invoice_of_record') {
                par.woa_flag = 0;
                par.invoice_flag = 1;
                par.record_invoice_flag = 1;
            }
            ...
            // 未销账 需票 未超期 小计 
            if (filed == 'unwoa_amount_of_commit_of_unlimit') {
                par.woa_flag = 0;
                par.invoice_flag = 1;
            ...
            // 未销账 需票 已超期 小计 
            if (filed == 'unwoa_amount_of_commit_of_limit') {
                par.woa_flag = 0;
                par.invoice_flag = 1;
                par.limit_fee_dat_flag = 1;
            }
           // $("#tab_fee_list").datagrid('load', par);

            $('#win_order_fee_group').window('close');
        }
    });
}
