...
//新建 账单 
function insert_ca_main() {
    $('#dlg_ed_ca_seq').val('');
    $('#dlg_ed_ca_cu_id').combogrid('enable');
    //当前参数中是否有 结算单位 
    if (!isNaN(cur_ca_list_query_params.ca_cu_id)) {
        var ca_cu_desc = $('#search_ca_cu_id').combogrid('getText');
        $('#dlg_ed_ca_cu_id').data('cu_id', cur_ca_list_query_params.ca_cu_id);
        $('#dlg_ed_ca_cu_id').combogrid('setText', $('#search_ca_cu_id').combogrid('getText'));
    }
    $('#dlg_ed_ca_title').val('');
    $('#dlg_ed_ca_group_flag').prop('checked', false);
    $('#dlg_ed_ca_bak').val('');
                ...
                handler: function () {
                    ...
                    function (data) {
                        if (data.result == 1) {
                            $('#dlg_ed_ca_seq').val(data.ca_seq);
                            refresh_ca_list();
                            $.messager.confirm('提示', data.ca_seq.length == 0 ? '新增完成，是否继续编辑？' : '修改完成，是否继续编辑？',
                            function (r) {
                                if (!r) {
                                    $('#dlg_new_ca_main').dialog('close');
                                } else {
...
function delete_ca_main() {
    ...
    */
    ...
    $.each(rows, function (i, row) {
        if (row.ca_status != 1) {
            bRight = false;
            msg = '错误:账单"' + row.ca_title + '"已交账，无法删除!';
            return;
        }
        if (row.ca_is_my_flag != 1) {
            bRight = false;
            msg = '错误:账单"' + row.ca_title + '"不是您创建的，您无权删除!'
            return;
        }
    ...
    $.messager.confirm('删除账单提示', '确定要删除"' + rows.length + '"条账单？',
    function (r) {
        if (r) {
            post('../Ashx/checkaccount.ashx', {
                rnd: Math.random(),
                action: 'delete_main_list',
                ca_seqs: ca_seqs
            }, function (data) {
                if (data.result == 1) {
                    $.messager.alert('提示', data.msg, 'info');
