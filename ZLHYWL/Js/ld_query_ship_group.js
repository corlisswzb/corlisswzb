...
var cur_query_ship_group_params = undefined;
...
function get_basesetting() {
    post('../Ashx/sys_base.ashx', {
        ...
        //bind_combogrid_custom($('#search_tools_owner_id'));
...
//刷新
function refresh_rpt_of_ship_group() {
    cur_query_ship_group_params = {
        rnd: Math.random(),
        action: 'get_rpt_of_ship_group', 
    ...
    cur_query_sub_params = $.extend(true,{},cur_query_ship_group_params);
 


    //只有月没有年，是不允许的 
    if (cur_query_ship_group_params.beg_month != undefined && Number(cur_query_ship_group_params.beg_month) > 0) {
        if (cur_query_ship_group_params.beg_year == undefined ||
            Number(cur_query_ship_group_params.beg_year) == 0) {
            $.messager.alert('错误','错误:无法理解的起始时间设置!','error');
            return;
        }
    }
    if (cur_query_ship_group_params.end_month != undefined && Number(cur_query_ship_group_params.end_month) > 0) {
        if (cur_query_ship_group_params.end_year == undefined ||
            Number(cur_query_ship_group_params.end_year) == 0) {
            $.messager.alert('错误', '错误:无法理解的截止时间设置!', 'error');
            return;
        }
    }
    
    post('../Ashx/lead_query.ashx', cur_query_ship_group_params,
    function (data) {
        $("#tab_ship_group").datagrid('loadData', data);
        $("#tab_ship_group").datagrid('reloadFooter', [
        {
            tools_desc: '',
            tools_no: '',
            tools_owner: '',
...
function init_tab_ship_group() { 
    ...
    $("#tab_ship_group").datagrid({
        ...
        checkOnSelect: true,
        columns: [[
            { field: 'tools_desc', title: '船名', align: 'center', width: 110, },
            { field: 'tools_no', title: '航次', align: 'center', width: 70 },
            { field: 'tools_owner', title: '船东', align: 'center', width: 110 },
            { field: 'order_count', title: '涉委托数', align: 'center', width: 90 }, 
        ...
        onRowContextMenu: function (e, field, row) {
            e.preventDefault();
            $('#mm_of_datagrid').data('tools_owner_id', row.tools_owner_id);
            $('#mm_of_datagrid').data('tools_desc', row.tools_desc);
            $('#mm_of_datagrid').data('tools_no', row.tools_no);
...
function download_ship_group() {
    var down_query_sub_params = $.extend(true,{}, cur_query_ship_group_params);
    down_query_sub_params.action = 'download_rpt_of_ship_group';
...
function win_view_of_order_list() {
    ...
    cur_query_sub_params.action = 'get_rpt_of_ship_group_of_order_list';
    cur_query_sub_params.od_route_tools_desc = $('#mm_of_datagrid').data('tools_desc');
    cur_query_sub_params.od_route_tools_no = $('#mm_of_datagrid').data('tools_no');
    cur_query_sub_params.od_route_tools_owner = $('#mm_of_datagrid').data('tools_owner_id');
...
function download_order_group() {
    ...
    down_query_sub_params.action = 'download_rpt_of_ship_group_order_list';
...
function win_view_of_order_fee_list(rec_or_pay) { 
    cur_query_sub_params.action = 'get_rpt_of_ship_group_of_fee_list';
...
function download_order_fee_group() {
    var down_query_sub_params = $.extend(true, {}, cur_query_sub_params);
    down_query_sub_params.action = 'download_rpt_of_ship_group_order_fee';
    window.open('../Ashx/lead_query.ashx?' + parseParams(down_query_sub_params));
}
...
function win_view_of_order_cntr_list() {
    ...
    cur_query_sub_params.action = 'get_rpt_of_ship_group_of_cntr_list';
    cur_query_sub_params.od_route_tools_desc = $('#mm_of_datagrid').data('tools_desc');
    cur_query_sub_params.od_route_tools_no = $('#mm_of_datagrid').data('tools_no');
    cur_query_sub_params.od_route_tools_owner = $('#mm_of_datagrid').data('tools_owner_id');
    post('../Ashx/lead_query.ashx', cur_query_sub_params,
...
function download_order_cntr_group() {
    var down_query_sub_params = $.extend(true, {}, cur_query_sub_params);
    down_query_sub_params.action = 'download_rpt_of_ship_group_order_cntr';
    window.open('../Ashx/lead_query.ashx?' + parseParams(down_query_sub_params));
}