...
function call_change_to_focus_model_of_ca() {
    ...
    $.cookie('bus_rec_page_ca_model', 'focus');
    //这里 应该 改一下 
    var content = '<iframe scrolling="auto" name="focus_page" frameborder="0"  src="template_ca_rec_for_bus_of_focus.aspx?rnd=' +
                        Math.random() +
                        '" style="width:100%;height:100%;"></iframe>';
    $('#panel_ca').panel({
        content: content
...
function call_change_to_normal_model_of_ca() {
    $.cookie('bus_rec_page_ca_model', 'normal');
    //这里 应该 改一下 
    var content = '<iframe scrolling="auto" name="normal_page" frameborder="0"  src="template_ca_rec_for_bus_of_normal.aspx?rnd=' +
                        Math.random() +
                        '" style="width:100%;height:100%;"></iframe>';
    $('#panel_ca').panel({
        content: content
...
function call_change_to_focus_model_of_fee() {
    $.cookie('bus_rec_page_fee_model', 'focus');
    //这里 应该 改一下 
    var content = '<iframe scrolling="auto" name="focus_page" frameborder="0"  src="template_order_fee_rec_fo_bus_focus.aspx?rnd=' +
                        Math.random() +
                        '" style="width:100%;height:100%;"></iframe>';
    $('#panel_fee').panel({
        content: content
...
function call_change_to_normal_model_of_fee() {
    $.cookie('bus_rec_page_fee_model', 'normal');
    //这里 应该 改一下 
    var content = '<iframe scrolling="auto" name="normal_page" frameborder="0"  src="template_order_fee_rec_fo_bus.aspx?rnd=' +
                        Math.random() +
                        '" style="width:100%;height:100%;"></iframe>';
    $('#panel_fee').panel({
        content: content
...
function get_basesetting() {
    post('../Ashx/sys_base.ashx', {
        ...
        //这里是切换 费用明细到 专注模式
        if ($.cookie('bus_rec_page_fee_model') == 'focus') {
            call_change_to_focus_model_of_fee();
        } else {
            call_change_to_normal_model_of_fee();
        }
        //这里是切换 账单模式到 专注模式
        if ($.cookie('bus_rec_page_ca_model') == 'focus') {
