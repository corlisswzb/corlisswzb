...
function call_change_to_focus_model() {
    $.cookie('opr_rec_page_model', 'focus');
    //这里 应该 改一下 
    var content = '<iframe scrolling="auto" frameborder="0"  src="template_order_fee_rec_fo_operation_focus.aspx?rnd=' +
                        Math.random() +
                        '" style="width:100%;height:100%;"></iframe>';
...
function call_change_to_normal_model() {
    $.cookie('opr_rec_page_model', 'normal');
    //这里 应该 改一下 
    var content = '<iframe scrolling="auto" frameborder="0"  src="template_order_fee_rec_fo_operation.aspx?rnd=' +
                        Math.random() +
                        '" style="width:100%;height:100%;"></iframe>';
    $('#pay_order_fee_list').panel({
        content: content
...
function get_basesetting() {
    post('../Ashx/sys_base.ashx', {
        ...
        if ($.cookie('opr_rec_page_model') == 'focus') {
...
function init_tab_ca_list() {
    ...
    $('#tab_ca_list').datagrid({
        ...
        checkOnSelect: true, 
        frozenColumns: [[
            { title: '', field: '_ck', rowspan:2, width: 40, checkbox: true }
        ]],
        columns: [[
                { title: '综合状态属性', colspan: 6, align: 'center' }
...
function init_tab_fee_list_of_ca() {
    ...
    $("#tab_fee_list_of_ca").datagrid({
        ...
        onRowContextMenu: function (e, field, row) {
            e.preventDefault();
            $('#win_dv_view_of_approval_details_from_list').data('od_amc_id', row.od_amc_id);
            $('#win_dv_view_of_approval_details_from_list').data('cu_id', row.fee_cu_id);
            $('#win_dv_view_of_approval_details_from_list').data('ca_seq', row.ca_seq);
            $('#win_dv_view_of_approval_details_from_list').data('ca_title', row.ca_title);
            $('#win_dv_view_of_approval_details_from_list').data('od_seq', row.od_seq);
            $('#win_dv_view_of_approval_details_from_list').data('od_no', row.od_no);
...
function post_ca_main() {
    ...
    if (rows.length > 1) {
        $.messager.alert('错误', '错误:单次只能投递一个', 'error');
        return;
    }
...
function table_bootom_selected_desc2(target_tab, rec_or_pay) {
    var rows = target_tab.datagrid('getData');
