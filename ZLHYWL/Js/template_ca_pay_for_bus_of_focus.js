...
function setting_checkaccount_count() {
    ...
    var setting = function (rows) {
        ...
        for(var i in rows){ 
            var text = '';
            if (i == 0) {
                text = '未交账(<span style="color:red">' + rows[i].rowscount + '<span>)';
                $('.top_group_menu').eq(i).linkbutton({
                    text: text
                });
            }
            if (i == 1) {
                text = '待收票(<span style="color:red">' + rows[i].rowscount + '<span>)';
                $('.top_group_menu').eq(i).linkbutton({
                    text: text
                });
            }
            if (i == 3) {
                text = '待交审(<span style="color:red">' + rows[i].rowscount + '<span>)';
                $('.top_group_menu').eq(2).linkbutton({
                    text: text
                });
            } 
        } 
