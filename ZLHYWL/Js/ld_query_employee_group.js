...
var cur_query_worker_group_params = undefined;
...
//刷新
function refresh_rpt_of_worker_group() {
    cur_query_worker_group_params = {
        rnd: Math.random(),
        action: 'get_rpt_of_worker_group',
        worker_user_typ: $('#search_worker_user_typ').combobox('getValue'),
    ...
    cur_query_sub_params = $.extend(true,{},cur_query_worker_group_params);
 


    //只有月没有年，是不允许的 
    if (cur_query_worker_group_params.beg_month != undefined && Number(cur_query_worker_group_params.beg_month) > 0) {
        if (cur_query_worker_group_params.beg_year == undefined ||
            Number(cur_query_worker_group_params.beg_year) == 0) {
            $.messager.alert('错误','错误:无法理解的起始时间设置!','error');
            return;
        }
    }
    if (cur_query_worker_group_params.end_month != undefined && Number(cur_query_worker_group_params.end_month) > 0) {
        if (cur_query_worker_group_params.end_year == undefined ||
            Number(cur_query_worker_group_params.end_year) == 0) {
            $.messager.alert('错误', '错误:无法理解的截止时间设置!', 'error');
            return;
        }
    }
    if (cur_query_worker_group_params.worker_user_typ == undefined || Number(cur_query_worker_group_params.worker_user_typ) > 3 || Number(cur_query_worker_group_params.worker_user_typ) < 1) {
        $.messager.alert('错误', '错误:类型条件,无法识别!', 'error');
        return;
    }
    post('../Ashx/lead_query.ashx', cur_query_worker_group_params,
    function (data) {
        $("#tab_employee_group").datagrid('loadData', data);
        $("#tab_employee_group").datagrid('reloadFooter', [
        {
            u_real_name: '', 
            order_count: parseInt( data.out_sum_of_order_count),
            sum_of_cargo_weight: parseFloat( data.out_sum_of_cargo_weight).toFixed(2),
            sum_of_cargo_packing_number: data.out_sum_of_cargo_packing_number,
            sum_of_cargo_bluk: parseFloat(data.out_sum_of_cargo_bluk).toFixed(2),
            ...
            sum_of_percent_profit: data.out_sum_of_percent_profit 
...
//初始表格
function init_tab_employee_group() { 
    cur_query_worker_group_params = {
        rnd: Math.random(),
        action: 'get_rpt_of_worker_group',
        worker_user_typ: $('#search_worker_user_typ').combobox('getValue'), 
    ...
    $("#tab_employee_group").datagrid({
        ...
        checkOnSelect: true,
        columns: [[
            { field: 'u_real_name', title: '姓名', align: 'center',width:60, },
            ...
            { field: 'sum_of_percent_profit', title: '毛利率', align: 'center', width: 160 },
        ]], 
        onRowContextMenu: function (e, field, row) {
            e.preventDefault();
            $('#mm_of_datagrid').data('u_id', row.u_id);
            $('#mm_of_datagrid').data('u_name', row.u_real_name);
...
function download_employee_group() {
    var down_query_sub_params = $.extend(true,{}, cur_query_worker_group_params);
    down_query_sub_params.action = 'download_rpt_of_worker_group';
...
function win_view_of_order_list() {
    cur_query_sub_params.u_id = $('#mm_of_datagrid').data('u_id');
    cur_query_sub_params.action = 'get_rpt_of_worker_group_of_order_list';
    cur_query_sub_params.u_name = $('#mm_of_datagrid').data('u_name');
    post('../Ashx/lead_query.ashx', cur_query_sub_params,
    function (data) {
        $("#tab_order").datagrid('loadData', data);
        $('#win_of_order_list').window({
            title: cur_query_sub_params.u_name + '关联订单列表',
        }).window('open');
    },true);
}
...
function download_order_group() {
    ...
    down_query_sub_params.action = 'download_rpt_of_order_list';
...
function win_view_of_order_fee_list(rec_or_pay) {
    ...
    cur_query_sub_params.u_id = $('#mm_of_datagrid').data('u_id');
    cur_query_sub_params.action = 'get_rpt_of_worker_group_of_fee_list';
    ...
    function (data) {
        ...
        $('#win_of_order_fee_list').window({
            title:cur_query_sub_params.u_name + "关联费用列表",
        }).window('open');
    }, true);
...
function download_order_fee_group() {
    var down_query_sub_params = $.extend(true, {}, cur_query_sub_params);
    down_query_sub_params.action = 'download_rpt_of_order_fee';
    window.open('../Ashx/lead_query.ashx?' + parseParams(down_query_sub_params));
}
...
function win_view_of_order_cntr_list() {
    cur_query_sub_params.u_id = $('#mm_of_datagrid').data('u_id');
    cur_query_sub_params.action = 'get_rpt_of_worker_group_of_cntr_list';
    cur_query_sub_params.u_name = $('#mm_of_datagrid').data('u_name');
    post('../Ashx/lead_query.ashx', cur_query_sub_params,
    function (data) {
        $("#tab_order_cntr").datagrid('loadData', data);
        $('#win_of_order_cntr_list').window({
            title: cur_query_sub_params.u_name + "关联集装箱列表",
        }).window('open');
    }, true);
}
...
function download_order_cntr_group() {
    var down_query_sub_params = $.extend(true, {}, cur_query_sub_params);
    down_query_sub_params.action = 'download_rpt_of_order_cntr';
    window.open('../Ashx/lead_query.ashx?' + parseParams(down_query_sub_params));
}